/* ===================== облако: ЗАГЛУШКА =====================
   ...
   В собранном плеере её нет вовсе - см. самый низ файла. */
...
// Это макет для разговора, а не функция, и людям его показывать нельзя:
// на сайте обещано «ни аккаунта», а окно входа тут понарошку. Поэтому
// в собранном плеере блоки просто выкидываются, и видно их только при
// запуске из исходников. Если спросить не вышло - тоже выкидываем:
// лучше не показать макет автору, чем показать его всем
(async () => {
  let packaged = true;
  try { packaged = (await window.api.info()).packaged !== false; } catch {}
  if (!packaged) { cloudWire(); return; }
  for (const id of ['cloud-block', 'cloud-win']) { const n = $(id); if (n) n.remove(); }
})();
