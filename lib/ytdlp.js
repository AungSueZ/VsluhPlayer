...
// YouTube прячет ссылки на звук за задачками на JS, и yt-dlp решает их чужим
// движком. Без движка анонимно ещё кое-как отдаётся, а с куками - уже нет:
// «Requested format is not available». Ставить людям Deno ради этого глупо -
// внутри нас уже сидит Node, его и одалживаем. ELECTRON_RUN_AS_NODE включает
// в нашем же exe чистый Node, без окон; yt-dlp сам запускает его с --permission,
// то есть без доступа к диску и сети
...
const HINTS = [
  // Куки идут первыми: их поломка выглядит как чужая ошибка про DPAPI,
  // и человеку решительно нечего с ней делать, пока ему не скажут словами.
  // Chrome с 127-й версии шифрует свои куки так, что снаружи их не прочитать
  // (yt-dlp #10927). То же у всех на его основе: Edge, Brave, Opera, Vivaldi.
  [/DPAPI|Failed to decrypt|decrypt with/i,
   'Chrome не отдаёт свои куки — в настройках выбери Firefox или «не использовать»'],
  [/could not copy|unable to (open|read)[^\n]*cookie|database is locked|Permission denied[^\n]*[Cc]ookies/i,
   'браузер держит куки занятыми — закрой его и попробуй ещё раз'],
  [/could not find[^\n]*cookies database|no such file[^\n]*[Cc]ookies/i,
   'у этого браузера не нашлось кук — выбери другой в настройках'],

  [/confirm your age|age-restricted|age restricted/i, 'YouTube просит подтвердить возраст — включи куки браузера в настройках (надёжнее всего Firefox)'],
  [/confirm you'?re not a bot|not a bot/i,           'YouTube принял нас за бота — включи куки браузера в настройках (надёжнее всего Firefox)'],
  // так YouTube говорит «слишком часто», и звучит это как «ролика нет».
  // Проходит само, обычно за час
  [/isn.?t available,? try again later|rate.?limit/i,  'YouTube на время закрыл скачивание — попробуй через час'],
  [/requested format is not available/i,              'YouTube не отдал звук — обнови yt-dlp в настройках или возьми другую загрузку трека'],
  ...
  [/copyright|blocked in your country|available in your country/i, 'заблокировано по региону или правообладателем'],
...
function run(exe, args, { onLine, timeout = 0, onSpawn } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const ps = spawn(exe, args, {
      windowsHide: true,
      env: Object.assign({}, process.env, { PYTHONIOENCODING: 'utf-8', PYTHONUTF8: '1', ELECTRON_RUN_AS_NODE: '1' })
    });
    let out = '', err = '', tail = '';
    let killed = false;
    ...
    ps.on('close', code => {
      ...
      const e = new Error(humanError(err || out) || ('код ' + code));
      // человеку - одна строка, а целиком - для лога: по одной строке
      // не понять, что на самом деле ответил YouTube
      e.raw = String(err || out).trim().slice(-4000);
      reject(e);
    });
  });
}
...
async function install(dir, onProgress) {
  ...
  // на Linux и macOS скачанный файл без права на запуск не запустится
