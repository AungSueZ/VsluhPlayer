...
const DEFAULTS = {
  ...
  lastTrack: null,
  view: 'home',         // куда попадаешь при первом запуске

  favorites: [],          // id любимых треков
  playlists: [],          // {id, name, tracks:[id], cover, color, author, desc}
  ...
  profile: {
    ...
    titleColor: '',
    // соц-сети: храним короткое имя, а адрес собираем сами из известных доменов,
    // чтобы по клику нельзя было увести человека куда попало
    links: { vk: '', tg: '', ds: '', sp: '', gh: '' }
  },
  ...
  theme: {
    ...
    motion: 'full',       // плавность интерфейса: full | calm | off
    side: 'normal',       // левый столб: icons | normal | wide
    sideQueue: true,      // показывать ли в нём очередь
  ...
  smtc: { on: true },
  mini: { x: null, y: null },   // где стоит окошко мини-плеера; null - правый нижний угол
